<?php
	...
	if (!empty($pesquisa) && $_GET['page'] == 'categoria')
	{
		$res_categoria = $obj_categoria->pesquisaCategoria(Login::codigoUsuario(), $pesquisa, $organizar, $limite);
		$total_registros = $obj_categoria->totalCategoria(Login::codigoUsuario(), $pesquisa);
	}
	else
	{
		$res_categoria = $obj_categoria->selectCategoria(Login::codigoUsuario(), $organizar, $limite);
		$total_registros = $obj_categoria->totalCategoria(Login::codigoUsuario());
	}
...
?>
<script>
	document.title = "Categoria - Valix";
</script>
<div class="row" style="margin:0;padding-top: 10px;padding-bottom: 10px;background:#272727 !important;">
	<div class="col-xs-6" style="padding:0;margin:0;" align="center">
		<button class="btn btn-success" type="button" style="margin:0;" onclick="adicionarCategoria();" style="padding-left:20px;padding-right:20px;"><i class="fa fa-plus" style="font-size:12px;"></i>
			<span>NOVO</span>
		</button>			
	</div>
	<div class="col-xs-6" style="padding:0;margin:0;" align="center">
		<div class="btn-group">
		    ...
		    <div class="dropdown-menu">
		        <a class="dropdown-item <?php if(empty($_GET['sort']) || $_GET['sort'] == 'A-Z'){ echo 'active';} ?>" href=".?page=categoria<?php echo (isset($_GET['pesquisa'])) ? "&pesquisa=".$_GET['pesquisa'] : ""; ?>&sort=A-Z">A - Z</a>
		        <a class="dropdown-item <?php if($_GET['sort'] == 'Z-A'){ echo 'active';} ?>" href=".?page=categoria<?php echo (isset($_GET['pesquisa'])) ? "&pesquisa=".$_GET['pesquisa'] : ""; ?>&sort=Z-A">Z - A</a>
		    </div>
		</div>		
	</div>
</div>
...
<?php } ?>
<table class="table table-bordered table-striped" border="1" id="tableCategorias">
	<tr style="display:none;" id="categoriaTrNova">
		<td style="width:50%;border-right:1px solid rgba(189, 189, 189, 8.26);border-top:0px !important;border-bottom:0px !important;">
			<input class="form-control input-sm" id="novaCategoria" placeholder="Nova Categoria" style="background: white;height: 25px !important;margin: 0;line-height: inherit !important;font-size: 14px;width: 100%;padding: 0;" onkeypress="$(this).val(aspas($(this).val()));" onkeyup="$(this).val(aspas($(this).val()));" />
		</td>
		<td style="width:50%;border-top:0px !important;border-bottom:0px !important;">
			<button class="btn btn-success btn-sm" style="padding:7px;margin:0;" onclick="novaCategoria();">
				<small>Salvar</small>
			</button>
			<button class="btn btn-warning btn-sm" style="padding:7px;margin:0;" onclick="cancelarCategoria();">
				<small>Cancelar</small>
			</button>
		</td>
	</tr>
	<?php foreach($res_categoria as $valor){ ?>
		<tr id='categoriaTr<?php echo $valor['codigo'] ?>'>
			<td style='width:50%; border-right:1px solid rgba(189, 189, 189, 8.26); border-top:0px !important;border-bottom:0px !important;'><small id='categoriaNome<?php echo $valor['codigo'] ?>'><?php echo $valor['nome'] ?></small></td>
			<td style='width:50%;border-top:0px !important;border-bottom:0px !important;'>
				<button class='btn btn-success btn-sm' id='categoriaSalvar<?php echo $valor['codigo'] ?>' style='padding:7px;margin:0;display:none;' onclick='salvarCategoria(<?php echo $valor['codigo'] ?>)'><small>Salvar</small></button>	
				<button class='btn btn-info btn-sm' id='categoriaEditar<?php echo $valor['codigo'] ?>' style='padding:7px;margin:0;' onclick='editarCategoria(<?php echo $valor['codigo'] ?>)'><small>Editar</small></button>
				<button class='btn btn-danger btn-sm' style='padding:7px;margin:0;' onclick='excluirCategoria(<?php echo $valor['codigo'] ?>)'><small>Excluir</small></button>
				<button class="btn btn-warning btn-sm" id="categoriaCancelar<?php echo $valor['codigo'] ?>" style="padding:7px;margin:0; display:none;" onclick="cancelarCategoria(<?php echo $valor['codigo'].", '".str_replace("'","\'",$valor['nome'])."'" ?>)"><small>Cancelar</small></button>
			</td>
		</tr>
	<?php } ?>
</table>
				...
				<p>
					<small>
						<small>
							...
							Total de Categorias: <?php echo $total_registros; ?>
						</small>
					</small>
				</p>
			</center>
