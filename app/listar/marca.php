<?php 
	...
	if (!empty($pesquisa) && $_GET['page'] == 'marca')
	{
		$res_marca = $obj_marca->pesquisaMarca(Login::codigoUsuario(), $pesquisa, $organizar, $limite);
		$total_registros = $obj_marca->totalMarca(Login::codigoUsuario(), $pesquisa);
	}
	else
	{
		$res_marca = $obj_marca->selectMarca(Login::codigoUsuario(), $organizar, $limite);
		$total_registros = $obj_marca->totalMarca(Login::codigoUsuario());
	}
...
<div class="row" style="margin:0;padding-top: 10px;padding-bottom: 10px;background:#272727 !important;">
	<div class="col-xs-6" style="padding:0;margin:0;" align="center">
		<button class="btn btn-success" type="button" style="margin:0;" onclick="adicionarMarca();" style="padding-left:20px;padding-right:20px;"><i class="fa fa-plus" style="font-size:12px;"></i>
			<span>NOVO</span>
		</button>			
	</div>
	<div class="col-xs-6" style="padding:0;margin:0;" align="center">
		<div class="btn-group">
		    ...
		    <div class="dropdown-menu">
		        <a class="dropdown-item <?php if(empty($_GET['sort']) || $_GET['sort'] == 'A-Z'){ echo 'active';} ?>" href=".?page=marca<?php echo (isset($_GET['pesquisa'])) ? "&pesquisa=".$_GET['pesquisa'] : ""; ?>&sort=A-Z">A - Z</a>
		        <a class="dropdown-item <?php if($_GET['sort'] == 'Z-A'){ echo 'active';} ?>" href=".?page=marca<?php echo (isset($_GET['pesquisa'])) ? "&pesquisa=".$_GET['pesquisa'] : ""; ?>&sort=Z-A">Z - A</a>
		    </div>
		</div>		
	</div>
</div>
...
<?php } ?>
<table class="table table-bordered table-striped" border="1" id="tableMarcas">
	<tr style="display:none;" id="marcaTrNova">
		<td style="width:50%;border-right:1px solid rgba(189, 189, 189, 8.26);border-top:0px !important;border-bottom:0px !important;">
			<input class="form-control input-sm" id="novaMarca" placeholder="Nova Marca" style="background: white;height: 25px !important;margin: 0;line-height: inherit !important;font-size: 14px;width: 100%;padding: 0;" onkeypress="$(this).val(aspas($(this).val()));" onkeyup="$(this).val(aspas($(this).val()));" />
		</td>
		<td style="width:50%;border-top:0px !important;border-bottom:0px !important;">
			<button class="btn btn-success btn-sm" style="padding:7px;margin:0;" onclick="novaMarca();">
				<small>Salvar</small>
			</button>
			<button class="btn btn-warning btn-sm" style="padding:7px;margin:0;" onclick="cancelarMarca();">
				<small>Cancelar</small>
			</button>
		</td>
	</tr>
	<?php foreach($res_marca as $valor){ ?>
		<tr id='marcaTr<?php echo $valor['codigo'] ?>'>
			<td style='width:50%; border-right:1px solid rgba(189, 189, 189, 8.26); border-top:0px !important;border-bottom:0px !important;'><small id='marcaNome<?php echo $valor['codigo'] ?>'><?php echo $valor['nome'] ?></small></td>
			<td style='width:50%; border-top:0px !important; border-bottom:0px !important;'>
				<button class='btn btn-success btn-sm' id='marcaSalvar<?php echo $valor['codigo'] ?>' style='padding:7px;margin:0;display:none;' onclick='salvarMarca(<?php echo $valor['codigo'] ?>)'><small>Salvar</small></button>
				<button class='btn btn-info btn-sm' id='marcaEditar<?php echo $valor['codigo'] ?>' style='padding:7px;margin:0;' onclick='editarMarca(<?php echo $valor['codigo'] ?>)'><small>Editar</small></button>
				<button class='btn btn-danger btn-sm' style='padding:7px;margin:0;' onclick='excluirMarca(<?php echo $valor['codigo'] ?>)'><small>Excluir</small></button>
				<button class="btn btn-warning btn-sm" id="marcaCancelar<?php echo $valor['codigo'] ?>" style="padding:7px;margin:0; display:none;" onclick="cancelarMarca(<?php echo $valor['codigo'].", '".str_replace("'","\'",$valor['nome'])."'" ?>)"><small>Cancelar</small></button>
			</td>
		</tr>
	<?php } ?>
</table>
				...
				<p>
					<small>
						<small>
							...
							Total de Marcas: <?php echo $total_registros; ?>
						</small>
					</small>
				</p>
			</center>
