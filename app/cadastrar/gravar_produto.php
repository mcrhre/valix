<?php 
	...
	if ($_POST){
		...
		$resul_produto = $produto_dao->insertProduto($produto);
		...
		if (array_key_exists('id', $resul_json)){
			...
			$aviso_produto->setProduto($codigo_produto);
			...
			$resul_aviso = $aviso_produto_dao->insertAvisoProduto($aviso_produto);
